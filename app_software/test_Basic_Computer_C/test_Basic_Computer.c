...
int	display_toggle = 0;					// toggles display of the text dE1 and PASSED

/***************************************************************************************
 * Main program: exercises various features of the DE1 basic computer, as a test 
...
int main(void)
{
	...
	*/
	...
	display_toggle = 0;					// display initialized to scroll the text dE1
	...
	*(pushbutton_ptr + 2) = 0xE; 		// Set the 3 interrupt mask bits to 1 (bit 0 is Nios II reset)
	...
	while ( !memory_error )
	{
		...
		if ( eight_sec > 80 )
		{
			...
			{
				display_toggle = 0;
				display_buffer[0] = 0xdE1;				// code for the word dE1
				display_buffer[1] = 0x0;		
				display_buffer[2] = 0x0;	
			}
		}
